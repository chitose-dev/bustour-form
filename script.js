...
function showReservationDetail(id) {
    ...
    body.innerHTML = ''
        ...
        + '</div>'
        + '<div><label class="block text-gray-600 text-sm mb-1">予約媒体</label><select id="edit-res-booking-source" class="w-full border p-2 rounded text-sm bg-white"><option value="LINE公式"' + (r.bookingSource === 'LINE公式' ? ' selected' : '') + '>LINE公式</option><option value="Web直接"' + (r.bookingSource === 'Web直接' ? ' selected' : '') + '>Web直接</option><option value="電話"' + (r.bookingSource === '電話' ? ' selected' : '') + '>電話</option></select></div>'
